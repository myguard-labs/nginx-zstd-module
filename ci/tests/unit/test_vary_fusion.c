 ...
 * real nginx headers in .build/nginx-1.31.4 (built by
 * ci/tools/nginx-tree.sh / the syntax-gate flow) -- not a hand-copied
 * decision-logic shim. Only ngx_list_push()/ngx_palloc() are stubbed
 * here, as trivial arena allocators, so the test needs no full nginx
 * process, no ngx_cycle, no config parse: the Vary helpers only ever
 * touch r->headers_out.headers, r->gzip_vary and one loc_conf pointer.
 *
...
make_request(ngx_flag_t gzip_vary)
{
    ...
    r = ngx_palloc(NULL, sizeof(ngx_http_request_t));
    memset(r, 0, sizeof(*r));

    clcf = ngx_palloc(NULL, sizeof(ngx_http_core_loc_conf_t));
    memset(clcf, 0, sizeof(*clcf));
    clcf->gzip_vary = gzip_vary;

    loc_conf = ngx_palloc(NULL, sizeof(void *) * 1);
    loc_conf[0] = clcf;
    r->loc_conf = loc_conf;
