...
>
> The one exception: if [ngx_http_compression_vary_filter_module](https://github.com/HanadaLee/ngx_http_compression_vary_filter_module) is loaded, that module emits (and flattens) `Vary: Accept-Encoding` from `r->gzip_vary` on its own, replacing the `gzip_vary` directive — `gzip_vary off` is then a valid configuration, and the config-load warnings are suppressed automatically whenever a module by that name is present.
