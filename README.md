...
| **Harness Fault Arms** ([`harness-fault-arms.yml`](.github/workflows/harness-fault-arms.yml)) | every PR (via CI), **not required** | Builds a separate nginx tree with the [`nginx-module-testkit`](https://github.com/myguard-labs/nginx-module-testkit) fault-injection harness compiled in (`TEST_HARNESS=1` + `-DNGX_TEST_HARNESS`, never the packaged build), then runs [`ci/t/harness-scenarios/fault-arms`](ci/t/harness-scenarios/fault-arms): injects a `ZSTD_compressStream2()` failure and a success-with-zero-output at both the mid-stream and end-of-frame call sites, asserting the request fails closed rather than serving corrupt output, and that the empty-output suppression path still completes a full response. Deliberately not a required check — see the workflow's own header. |
